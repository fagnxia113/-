[DEAFULTGP]
Name=调研活动
ShowName=调研活动
CmdNum=9
GPSetCode_Code1=1_603739
GPSetCode_Code2=1_600000
GPSetCode_Code3=0_300001
GPSetCode_Code4=1_688001
GPSetCode_Code5=1_600804
GPSetCode_Code6=1_600805
GPSetCode_Code7=0_000012
GPSetCode_Code8=1_600807
GPSetCode_Code9=1_600808
GPSetCode_Code10=1_600809
GPSetCode_Code11=1_600810
GPSetCode_Code12=1_600811
GPSetCode_Code13=1_600812
GPSetCode_Code14=0_002241
GPSetCode_Code15=1_600814
GPSetCode_Code16=1_600815
GPSetCode_Code17=1_600816
GPSetCode_Code18=1_600817
GPSetCode_Code19=1_600818
GPSetCode_Code20=1_600819
GPSetCode_Code21=0_000027
UnitNum=1
KeyGuyToExtern=0
ForceUseDS=0
PadMaxCx=0
PadMaxCy=0
PadHelpStr=个股调研排名>>
PadHelpUrl=http://www.treeid/syszdpadcode_ggdy
HasProcessBtn=0
UnSizeMode=0
HQGridNoCode=0
crTipWord=0
FixedSwitchMode=0
AutoFitMode=0
UserPadFlag=0
RelType=0
RelType2=0
RelType1For2=0
RelType2For1=0
CTPGroupType=0
AutoSize=0
[STEP0]
SplitWhich=-1
UnitStr=通用浏览器
UnitStr_Long=
UnitDesc=
UnitGlStr=
UnitType=WEB_UNIT
UnitStyle=ZST_BIG
HowToSplit=0
SplitRatio=100.0000
ShowGpNo=1
IsLocked=0
Fixed_Width=0
Fixed_Height=0
IsCurrent=1
OneCanShowSwitch=0
ShowRefreshBtn=0
SwitchBarPos=0
SwitchBarScheme=0
CurUrlSel=0
Url=http://##tdxcloudaddress_tp##/site/tdx-pc-zx/page-index.html?color=##tdxbk##&bkcolor=##tdxbkcolor##&menu=dy
ZSCheck=1
AutoRefreshMin=0
ForceUseIECheck=0
