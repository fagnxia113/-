...
const PORTFOLIO_SELECT_CLASS = `${PORTFOLIO_INPUT_CLASS} appearance-none pr-8`;
const PORTFOLIO_FILE_PICKER_CLASS =
  'input-surface input-focus-glow flex h-8 w-full cursor-pointer items-center justify-center rounded-sm border bg-transparent px-3 text-xs font-mono transition-all focus:outline-none disabled:cursor-not-allowed disabled:opacity-60';
...
const PortfolioPage: React.FC = () => {
  ...
  return (
    <div className="portfolio-page min-h-screen space-y-2 p-3">
      <section className="space-y-2">
        <div className="space-y-1">
          <h1 className="text-base font-semibold text-foreground">持仓管理</h1>
          <p className="text-[11px] text-secondary">
            组合快照、手工录入、CSV 导入与风险分析
          </p>
        </div>
        {hasAccounts ? (
          <div className="rounded-sm border border-border p-2">
            <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,1fr)_180px_240px] gap-2 items-end">
              <div>
                <p className="text-[11px] text-secondary mb-1">账户视图</p>
                <select
              ...
              <div>
                <p className="text-[11px] text-secondary mb-1">成本口径</p>
                <select
                  value={costMethod}
          ...
          <InlineAlert
            variant="warning"
            className="inline-block rounded-sm px-3 py-2 text-xs shadow-none"
      ...
      {(showCreateAccount || !hasAccounts) ? (
        <Card padding="sm">
          <div className="flex items-center justify-between gap-2">
            <h2 className="text-xs font-semibold text-foreground">新建账户</h2>
            {hasAccounts ? (
              <button
                type="button"
              ...
              <span className="text-[11px] text-secondary">创建后自动切换到该账户</span>
            )}
          </div>
          {accountCreateError ? (
            <InlineAlert
              variant="danger"
              className="mt-2 rounded-sm px-2 py-1 text-xs shadow-none"
              title="创建账户失败"
          ...
          <form className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-1.5" onSubmit={handleCreateAccount}>
            <input
            ...
            <button type="submit" className="btn-secondary text-xs" disabled={accountCreating}>
      ...
      <section className="grid grid-cols-2 xl:grid-cols-4 gap-2">
        <Card variant="bordered" padding="sm">
          <p className="text-[11px] text-secondary">总权益</p>
          <p className="mt-0.5 text-sm font-semibold font-mono text-foreground">{formatMoney(snapshot?.totalEquity, snapshot?.currency || 'CNY')}</p>
        </Card>
        <Card variant="bordered" padding="sm">
          <p className="text-[11px] text-secondary">总市值</p>
          <p className="mt-0.5 text-sm font-semibold font-mono text-foreground">{formatMoney(snapshot?.totalMarketValue, snapshot?.currency || 'CNY')}</p>
        </Card>
        <Card variant="bordered" padding="sm">
          <p className="text-[11px] text-secondary">总现金</p>
          <p className="mt-0.5 text-sm font-semibold font-mono text-foreground">{formatMoney(snapshot?.totalCash, snapshot?.currency || 'CNY')}</p>
        </Card>
        <Card variant="bordered" padding="sm">
          <div className="flex items-start justify-between gap-2">
            <p className="text-[11px] text-secondary">汇率状态</p>
            <button
              type="button"
          ...
          <div className="mt-1">{snapshot?.fxStale ? <Badge variant="warning">过期</Badge> : <Badge variant="success">最新</Badge>}</div>
          {fxRefreshFeedback ? (
            <InlineAlert
              variant={getFxRefreshFeedbackVariant(fxRefreshFeedback.tone)}
              title="汇率刷新结果"
              message={fxRefreshFeedback.text}
              className="mt-2 rounded-sm px-2 py-1 text-[11px] shadow-none"
            />
          ) : null}
        </Card>
      </section>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-2">
        <Card className="xl:col-span-2" padding="sm">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-xs font-semibold text-foreground">持仓明细</h2>
            <span className="text-[11px] text-secondary">共 {positionRows.length} 项</span>
          </div>
          {positionRows.length === 0 ? (
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead className="text-[11px] text-secondary border-b border-border">
                  <tr>
                    <th className="text-left py-1.5 pr-2">账户</th>
                    <th className="text-left py-1.5 pr-2">代码</th>
                    <th className="text-right py-1.5 pr-2">数量</th>
                    <th className="text-right py-1.5 pr-2">均价</th>
                    <th className="text-right py-1.5 pr-2">现价</th>
                    <th className="text-right py-1.5 pr-2">市值</th>
                    <th className="text-right py-1.5 pr-2">未实现盈亏</th>
                ...
                <tbody>
                  {positionRows.map((row) => (
                    <tr key={`${row.accountId}-${row.symbol}-${row.market}`} className="border-b border-border/30">
                      ...
                      <td className="py-1.5 pr-2 text-right font-mono">{row.quantity.toFixed(2)}</td>
                      <td className="py-1.5 pr-2 text-right font-mono">{row.avgCost.toFixed(4)}</td>
                      <td className="py-1.5 pr-2 text-right font-mono">
                        <div>{formatPositionPrice(row)}</div>
                        <div className={`text-[10px] ${hasPositionPrice(row) ? 'text-secondary' : 'text-warning'}`}>
                          {getPositionPriceLabel(row)}
                        </div>
                      </td>
                      <td className="py-1.5 pr-2 text-right font-mono">{formatPositionMoney(row.marketValueBase, row)}</td>
                      <td
        ...
        <Card padding="sm">
          <h2 className="text-xs font-semibold text-foreground mb-2">{concentrationMode === 'sector' ? '行业集中度分布' : '个股集中度'}</h2>
          ...
          <div className="mt-2 text-[11px] text-secondary space-y-0.5 font-mono">
            <div>展示口径: {concentrationMode === 'sector' ? '行业维度' : '个股维度'}</div>
      ...
      {writeBlocked && hasAccounts ? (
        <InlineAlert
          variant="warning"
          className="rounded-sm px-3 py-2 text-xs shadow-none"
      ...
      <section className="grid grid-cols-3 gap-2">
        <Card padding="sm">
          <h3 className="text-xs font-semibold text-foreground mb-1">回撤监控</h3>
          <div className="text-[11px] text-secondary space-y-0.5 font-mono">
        ...
        <Card padding="sm">
          <h3 className="text-xs font-semibold text-foreground mb-1">止损接近预警</h3>
          <div className="text-[11px] text-secondary space-y-0.5 font-mono">
        ...
        <Card padding="sm">
          <h3 className="text-xs font-semibold text-foreground mb-1">口径</h3>
          <div className="text-[11px] text-secondary space-y-0.5 font-mono">
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-2">
        <Card padding="sm">
          <h3 className="text-xs font-semibold text-foreground mb-2">手工录入：交易</h3>
          <form className="space-y-1.5" onSubmit={handleTradeSubmit}>
            ...
            <p className="text-[11px] text-secondary">手续费和税费可留空，系统将按 0 处理。</p>
            <button type="submit" className="btn-secondary w-full text-xs" disabled={!writableAccountId}>提交交易</button>
          </form>
        </Card>

        <Card padding="sm">
          <h3 className="text-xs font-semibold text-foreground mb-2">手工录入：资金流水</h3>
          <form className="space-y-1.5" onSubmit={handleCashSubmit}>
            ...
            <button type="submit" className="btn-secondary w-full text-xs" disabled={!writableAccountId}>提交资金流水</button>
          </form>
        </Card>

        <Card padding="sm">
          <h3 className="text-xs font-semibold text-foreground mb-2">手工录入：公司行为</h3>
          <form className="space-y-1.5" onSubmit={handleCorporateSubmit}>
            ...
            <button type="submit" className="btn-secondary w-full text-xs" disabled={!writableAccountId}>提交企业行为</button>
          </form>
        </Card>
      </section>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-2 gap-2">
        <Card padding="sm">
          <h3 className="text-xs font-semibold text-foreground mb-2">券商 CSV 导入</h3>
          <div className="space-y-1.5">
            {brokerLoadWarning ? (
              <InlineAlert
                variant="warning"
                className="rounded-sm px-2 py-1 text-xs shadow-none"
            ...
            <div className="flex gap-1.5">
              <button type="button" className="btn-secondary flex-1 text-xs" disabled={!csvFile || csvParsing} onClick={() => void handleParseCsv()}>
                {csvParsing ? '解析中...' : '解析文件'}
              </button>
            ...
            {csvParseResult ? (
              <InlineAlert
                ...
                className="rounded-sm px-2 py-1 text-[11px] shadow-none"
              />
            ) : null}
            {csvCommitResult ? (
              <InlineAlert
        ...
        <Card padding="sm">
          <h3 className="text-xs font-semibold text-foreground mb-2">事件记录</h3>
          <div className="space-y-1.5">
            <div className="grid grid-cols-2 gap-1.5">
              ...
              <button type="button" className="btn-secondary text-xs" onClick={() => void loadEvents()} disabled={eventLoading}>
            ...
            <div className="max-h-64 overflow-auto rounded-sm border border-border p-1.5">
              {eventType === 'trade' && tradeEvents.map((item) => (
                <div key={`t-${item.id}`} className="flex items-start justify-between gap-2 border-b border-border/30 py-1.5 text-[11px] text-secondary font-mono">
                  ...
                  {!writeBlocked ? (
                    <button
                      type="button"
                      className="btn-secondary shrink-0 !px-2 !py-0.5 !text-[10px]"
              ...
              {eventType === 'cash' && cashEvents.map((item) => (
                <div key={`c-${item.id}`} className="flex items-start justify-between gap-2 border-b border-border/30 py-1.5 text-[11px] text-secondary font-mono">
                  <div className="min-w-0">
              ...
              {eventType === 'corporate' && corporateEvents.map((item) => (
                <div key={`ca-${item.id}`} className="flex items-start justify-between gap-2 border-b border-border/30 py-1.5 text-[11px] text-secondary font-mono">
                  <div className="min-w-0">
            ...
            <div className="flex items-center justify-between text-[11px] text-secondary">
              <span className="font-mono">第 {eventPage} / {totalEventPages} 页</span>
              <div className="flex gap-1.5">
                <button type="button" className="btn-secondary text-[11px] px-2 py-0.5" disabled={eventPage <= 1}
                  onClick={() => setEventPage((prev) => Math.max(1, prev - 1))}>
                  上一页
                </button>
                <button type="button" className="btn-secondary text-[11px] px-2 py-0.5" disabled={eventPage >= totalEventPages}
