...
CN_BLUEPRINT = MarketStrategyBlueprint(
    ...
    dimensions=[
        ...
        StrategyDimension(
            ...
            checkpoints=["涨跌家数与涨跌停结构", "成交额是否扩张", "高位股是否出现分歧", "连板高度与封板率变化"],
        ),
        StrategyDimension(
            name="政策与资金",
            objective="评估政策环境与增量资金对市场方向的影响。",
            checkpoints=[
                "近期是否有重大政策发布或预期变化（降准降息/IPO/印花税等）",
                "两融余额是否持续增减",
                "新基金发行是否活跃",
                "行业政策是否有新催化（新能源/AI/房地产等）",
            ],
        ),
        StrategyDimension(
            name="主线板块",
            objective="提炼可交易主线与规避方向。",
            checkpoints=["领涨板块是否具备事件催化", "板块内部是否有龙头带动", "领跌板块是否扩散", "板块轮动处于启动期还是退潮期"],
        ),
    ],
    action_framework=[
        "进攻：指数共振上行 + 成交额放大 + 主线强化 + 政策面偏暖。",
        "均衡：指数分化或缩量震荡，控制仓位并等待确认。",
        "防守：指数转弱 + 领跌扩散 + 政策面偏紧，优先风控与减仓。",
    ],
)
